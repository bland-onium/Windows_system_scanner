# WINDOWS SCANNER & OPTIMIZER

half-testing mode. ATTENTION
from last update Checker got a lot of silent updates: it lost functions of enhancing of PC and installing Win\Office licenses but instead it got:
- Multi-Thread
- Logical log-writing engine
- protection from writing logs on another USB
- NO admin rules required
- Now everything stay in one .bat file instead of .ps1 and .bat together
- It got stronger USB, ViPNet and Object searching engines
- It works on 70% DOS-systems (not Win XP, but 7, 8, 8.1, 10 and 11 work straight)

This script made for automatical scanning system components, seeking for licenses, adding more optimisation in system and 
Программа создана для автоматизированного сканирования системных компоненов, обнаруживания лицензий и всей необходимой информации.э

Now PSChecker weight much more because of Base64-encoded MAS_AIO.cmd which is lying as string more than in 100000 symbols

### Способ развёртывания:
Качаем файлы PSChecker.ps1 и PSChecker.bat, кладём их в одну папку и запускаем .bat файл от имени администратора
Программа спросит - хотим ли мы устанавливать лицензии на Винду и Офис, после реакции пользователя просканирует всё, что должна сканировать

Текущий список сканирования:
MS Windows
MS Office
BIOS
CPU (Процессор)
GPU (VRAM, Видеокарта)
RAM (ОЗУ, Оперативка)
Disks
Network Adapter
Network Settings
Monitors
Peripherals (including USB Flashes)
List of installed programs (Now it scans all programs, connected to registry | Пока что сканирует всё, что подвязано к реестру. В дальнейшем планируется введение более широкого функционала с привлечением системных вызовов на ЯП низкого уровня)

### Новости обновлений
5-я версия получила не так много обновлений, но улучшила функциональность:
- Сканер-Активатор MS и Office работает стабильнее и надёжнее
- MAS теперь запускается локально и не обращается к серверу (Антивирус всё равно надо выключать)
- Появилась функция сканирования мониторов
- Появилась функция сканирования периферии
- Появилась функция поиска ViPNeT
- Появилась функция оптимизации системы (Убирает телеметрию, отключает ненужные службы, отключает поиск, сильно притормаживает MS Edge)

4-я версия приобрела ещё одно ведрышко апдейтов, конкретно:
- Теперь есть выбор вывода сканирования девайсов
- Появилось сканирование сетевого адаптера и сетевых настроек
- Сканер программ сканирует только в одном режиме и ищет все программы на пк
- Добавлена функция оптимизации системы, которая выключает часть беполезных/неэффективных функций, а также отключает Windows Defender
- Сделан механизм автоматического закрывания программы
- (надеюсь, что это правда) Исправлена ошибка записи лога на другую флешку. Теперь всё должно записываться на ту флешку, с которой программа была запущена

### Задачи на будущее:
- [ ] Сортировка сканируемых программ
- [ ] Поиск программ по определённым названиям и параметрам
- [ ] Глубокий поиск лицензий на ПО

A lot of time ago it were planned as windows and office activator which will be used by user but at once i understood that this is not enough and now there is multifunctional script which scans system, creates logs and installs licenses on Windows and Office
